...
```{r}
journals <- read.csv("Table S1_Pred pubs.csv",stringsAsFactors = FALSE)
names(journals)
```
...
```{r}
j1.lme<-lme(lnYearsRunning~ Predatory.,random=~1|Journal,data=journals)
anova(j1.lme)
```

```{r}
j1.gls<-gls(lnYearsRunning~ Predatory.,data=journals)
anova(j1.lme,j1.gls)
```
...
```{r}
j2.lme<-lme(lnAvgPapersPerYear~ Predatory.,random=~1|Journal,data=journals)
anova(j2.lme)
```

```{r}
j2.gls<-gls(lnAvgPapersPerYear~ Predatory.,data=journals)
anova(j2.lme,j2.gls)
```

lme and AIC/LogLikelihood for effect of journal [random] and journal type [fixed] on open access APC
```{r}
j3.lme<-lme(lnOA_APC~Predatory.,random=~1|Journal,data=journals,na.action = "na.omit")
anova(j3.lme)
```

```{r}
j3.gls<-gls(lnOA_APC~Predatory.,data=journals,na.action = "na.omit")
anova(j3.lme,j3.gls)
```
